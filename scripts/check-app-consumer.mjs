...
const run = (cmd, args) =>
  execFileSync(cmd, args, { cwd: temp, stdio: "inherit" });
...
try {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "lai-app-consumer", private: true, type: "module" }),
  );
  run("npm", [
    "install",
    "--no-audit",
  ...
  assert(
    !Object.keys(installed.dependencies).some((name) =>
      /^(zustand|i18next|react-i18next|@tanstack\/|@better-fetch\/)/.test(name),
    ),
  );
  for (const name of [
    ...
    assert(
      !existsSync(join(temp, "node_modules", name)),
      `Dependency must stay inside LAI: ${name}`,
    );
  }
  ...
  cpSync(
    join(root, "tests/app-i18n-runtime.mjs"),
    join(temp, "i18n-runtime.mjs"),
  );
  cpSync(
    join(root, "tests/app-platform-runtime.mjs"),
    join(temp, "platform-runtime.mjs"),
  );
  run("node", [
    "--test",
    "runtime.mjs",
    "i18n-runtime.mjs",
    "platform-runtime.mjs",
  ]);
  ...
  for (const viewport of [
    ...
    assert.equal(
      requests.filter((r) => r.startsWith("GET /api/contacts?")).length,
      1,
      "Loader and hook share Query cache",
    );
    await page
      .getByRole("textbox", { name: "Rascunho" })
      .fill("Preferência local");
    await page.reload();
    await expect(page.getByRole("textbox", { name: "Rascunho" })).toHaveValue(
      "Preferência local",
    );
    ...
    assert.equal(
      requests.filter((r) => r === "PATCH /api/contacts/1").length,
      1,
    );
    await page
      .getByRole("button", { name: "Encerrar sessão de exemplo" })
      .click();
    await expect(page.getByRole("status")).toContainText("Sessão encerrada");
    await expect
      .poll(() =>
        page.evaluate(() => localStorage.getItem("lai-app-example-draft")),
      )
      .toBeNull();
    ...
    const failedRequests = requests.filter(
      (r) => r === "GET /api/contacts/404",
    ).length;
    assert.equal(failedRequests, 1, "404 is not retried automatically");
    await page.getByRole("button", { name: "Tentar novamente" }).click();
    await expect
      .poll(() => requests.filter((r) => r === "GET /api/contacts/404").length)
      .toBe(2);
    await expect(page.getByRole("alert")).toContainText(
      "Não foi possível carregar esta página",
    );
    ...
    await expect(
      page.getByRole("heading", { name: "Internacionalização", exact: true }),
    ).toBeVisible();
    await page.getByRole("combobox", { name: "Language" }).selectOption("en");
    await expect(
      page.getByRole("heading", { name: "Internationalization", exact: true }),
    ).toBeVisible();
    ...
    await expect(page.getByTestId("isolated")).toHaveText(
    ...
    await expect(
      defaults.getByRole("button", { name: "Next page", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Custom next", exact: true }),
    ).toBeVisible();
    await defaults
      .getByRole("checkbox", { name: "Select row 1", exact: true })
      .check();
    await expect(
      defaults.getByText("1 selected", { exact: true }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /^Range:/ })).toContainText(
      "Jan",
    );
    ...
    await expect(
      page.getByRole("button", { name: "Clear date range", exact: true }),
    ).toBeVisible();
    ...
    await expect(
      defaults.getByRole("button", { name: "Página siguiente", exact: true }),
    ).toBeVisible();
    await page.getByRole("button", { name: /^Range:/ }).click();
    await expect(
      page.getByRole("button", { name: "Ir al mes siguiente", exact: true }),
    ).toBeVisible();
    await page.keyboard.press("Escape");
    ...
    for (const [key, position, order] of [
      ...
      await expect(
        sortable.getByRole("button", { name: "Move Ana" }),
    ...
    await expect(
      page.locator('[data-role="assistant"] [data-part="text"]'),
    ).toHaveText("Integração pronta.");
    await expect(
      page.locator('[data-role="assistant"] [data-part="thinking"]'),
    ).toHaveText("Verificando a base.");
    await expect(page.locator('[data-part="tool-result"]')).toContainText(
      "true",
    );
  ...
  devServer = spawn(
    process.execPath,
    [
      join(temp, "node_modules/vite/bin/vite.js"),
      "--host",
      "127.0.0.1",
      "--port",
      String(devPort),
  ...
  console.log(
    "LAI app: isolated install, generated routes, strict types, runtime contracts and desktop/mobile browser flows passed.",
  );
} finally {
  await browser?.close();
