...
export async function createAppI18n({
  ...
  detectLanguage = true,
  ...options
}: AppI18nOptions = {}): Promise<AppI18n> {
  ...
  try {
    preferenceStorage ??=
      storageKey && typeof window !== "undefined"
        ...
        : undefined;
    saved = storageKey
      ? (preferenceStorage?.getItem(storageKey) ?? undefined)
      : undefined;
  } catch {
    /* Storage may be disabled; translation still works. */
  }
  const supported =
    options.supportedLngs === false
      ? undefined
      : (options.supportedLngs ?? ["pt-BR", "en", "es"]);
  const candidates = [
    saved,
    ...(detectLanguage &&
  ...
  const detected = candidates
    ...
    .map(
      (value) =>
        ...
        supported?.find(
          (language) => language.split("-")[0] === value.split("-")[0],
        ) ??
  ...
  for (const language of new Set([
    ...Object.keys(laiTranslations),
    ...Object.keys(callerResources),
  ])) {
    const overrides = callerResources[language]?.lai;
    resources[language] = {
      ...callerResources[language],
      lai: {
        ...laiTranslations[language as keyof typeof laiTranslations],
  ...
  const persistLanguage = (language: string) => {
    try {
      if (storageKey) preferenceStorage?.setItem(storageKey, language);
    } catch {
  ...
  return Object.assign(instance, {
    dispose: () => instance.off("languageChanged", persistLanguage),
  });
}
