...
export function FormTextField({
  ...
  const errors = field.state.meta.isTouched
    ? field.state.meta.errors.map(formErrorMessage).filter(Boolean)
    : [];
  return (
    ...
    >
      {([canSubmit, busy]) => (
        <Button
          {...props}
          type="submit"
          disabled={props.disabled || !canSubmit || busy}
          aria-busy={busy}
        />
      )}
    </form.Subscribe>
...
export function formApiErrors<TName extends string>(
  fields: Partial<Record<TName, string>>,
  form?: string,
) {
