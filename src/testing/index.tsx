...
export async function createLaiTestContext(
  options: Parameters<typeof createAppI18n>[0] = {},
): Promise<LaiTestContext> {
  ...
  function Wrapper({ children }: { children: ReactNode }) {
    return (
      <QueryClientProvider client={queryClient}>
        <LaiI18nProvider i18n={i18n} updateDocument={false}>
          {children}
        </LaiI18nProvider>
      </QueryClientProvider>
    );
...
export async function renderWithLai(
  ui: ReactElement,
  options: Omit<RenderOptions, "wrapper"> & {
    i18n?: Parameters<typeof createAppI18n>[0];
  } = {},
): Promise<RenderResult & LaiTestContext> {
...
export const createTestRouter: typeof createRouter = (options) =>
  createRouter({
    ...options,
    history: options.history ?? createMemoryHistory({ initialEntries: ["/"] }),
  });
/** Capture the initial state of a store created for this test; restores persisted middleware too. */
export function createStoreReset<T>(store: {
  getInitialState(): T;
