...
export function SortableList<T>({
  ...
  const announcement = (id: UniqueIdentifier, over?: UniqueIdentifier) =>
    `${itemLabel(id)}: ${over === undefined ? "" : `${ids.indexOf(over) + 1}/${items.length}`}`;
  return (
    <DndContext
      sensors={sensors}
      collisionDetection={closestCenter}
      accessibility={{
        screenReaderInstructions: { draggable: instructions[lang] },
        announcements: {
          ...
          onDragCancel: ({ active }) =>
            `${itemLabel(active.id)}: ${lang === "en" ? "Cancelled" : lang === "es" ? "Cancelado" : "Cancelado"}`,
        },
      }}
      onDragEnd={({ active, over }) => {
        if (!over || active.id === over.id) return;
        const from = ids.indexOf(active.id),
          to = ids.indexOf(over.id);
