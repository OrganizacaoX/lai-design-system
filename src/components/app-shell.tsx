      ...
      <SidebarGroup className="p-0">
        ...
        <SidebarMenu className="gap-1.5">
          {group.items.map((item) => (
            <SidebarMenuItem key={item.id}>{renderItem(item)}</SidebarMenuItem>
          ))}
        </SidebarMenu>
      </SidebarGroup>
    );
      ...
      <SidebarMenu>
        ...
        >
          <CollapsibleTrigger
            ...
            className="group/nav-section min-h-11"
          >
            {group.icon}
...
function AppShellContent({
  ...
  const { isMobile, openMobile, setOpenMobile, toggleSidebar, state } =
    useSidebar();
  const contentId = useId();
  const activeIds = navigation
  ...
  const labels = {
    ...
    toggleNavigation: t("nav.toggle"),
    menu: t("nav.menu"),
    skipToContent: t("nav.skip"),
    ...overrides,
  };
  ...
  const link = (item: AppShellNavItem, mobile = false, sub = false) => {
    ...
    return sub ? (
      ...
      <SidebarMenuButton
        ...
        className={cn(
          "min-h-11",
          mobile && "h-14 min-h-14 justify-center rounded-full [&_svg]:size-5",
        )}
        onClick={closeDrawer}
      >
        {content}
  ...
  return (
    <>
      <a
        href={`#${contentId}`}
        className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-background focus:p-3 focus:ring-2"
      ...
      {bottom && (
        ...
        >
          ...
          <Button
            variant="ghost"
            className="h-14 min-h-14 flex-1 rounded-full [&_svg]:size-5"
