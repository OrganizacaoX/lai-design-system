...
export function SidebarProfile({
  ...
  const labels: SidebarProfileLabels = {
    trigger: `Perfil de ${user.name}`,
    profile: "Meu perfil",
    organization: "Trocar organização",
  ...
  const action = (
    value: SidebarProfileAction | undefined,
    label: string,
    icon: ReactNode,
  ) =>
    value && (
  ...
  const hasActions =
    ...
    install ||
    theme ||
    (language && language.options.length > 0) ||
    actions.length > 0;
  return (
          ...
          >
            ...
            {signOut && (
              <>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  disabled={signOut.pending || signOut.disabled}
                  onClick={signOut.onSelect}
                  aria-busy={signOut.pending || undefined}
                  className="text-destructive"
                >
                  <LogOut aria-hidden="true" />
                  {signOut.pending ? labels.signingOut : labels.signOut}
                </DropdownMenuItem>
              </>
            )}
          </DropdownMenuContent>
