export const usageGuidance: Record<string, string> = {
  "sidebar-profile":
    "Use no footer do AppShell ou dentro de SidebarFooter com SidebarProvider. Forneça apenas as ações disponíveis ao usuário: perfil, organização, instalação e saída. Tema e idioma são controlados pelo produto. O componente adapta avatar e identificação à sidebar recolhida e ao drawer mobile. Configure labels para traduzir e signOut.pending durante a saída; trate erros no produto.",
  shimmer:
    "Use em mensagens curtas de processamento ou geração. active=false remove o efeito; duration define milissegundos e once limita a uma passagem. Prefers-reduced-motion mostra texto estático. Para anúncios de progresso use role=status; não duplique regiões de anúncio. O pacote inclui a classe shimmer, e o registry instala o CSS necessário ao componente.",
  typography:
    "Escolha Title, SubTitle, SectionTitle e SectionSubTitle pela hierarquia do documento. Use Text para leitura, Lead para introduções e Description/Caption para apoio. Label identifica campos de formulário. Listas recebem li nativos; tabelas usam os componentes Table existentes. Todos os componentes aceitam props nativas, ref e className. As fontes e cores vêm do tema LAI. Ajuste o espaço externo com classes; não use Caption ou Description para esconder informação essencial.",
  "app-shell":
    "Use como layout raiz das rotas autenticadas. Filtre os itens por permissão no produto e informe active a partir do roteador. renderLink integra links do roteador; mobile marca até três atalhos inferiores, e Menu mantém acesso a todos os itens e ao rodapé. Use mobileNavigation=drawer para apenas o painel lateral. Marca, usuário, alertas e rótulos são configuráveis. O botão de recolher fica na sidebar; a área de conteúdo não tem cabeçalho embutido. Veja o exemplo completo em /examples/app-shell.",
  "app-update-card":
    "Use quando o app detectar uma versão nova: o card avisa sem bloquear a tela e só atualiza no clique da pessoa. Passe description quando atualizar naquele momento custar algo (uma ligação, um formulário aberto). A tela de progresso assume o app depois do clique; use brand para a marca do produto, que aparece também na variante failed, e informe onClearCache apenas quando o app tiver uma saída além do retry. A detecção da versão é do produto — o design system só entrega a interface.",
  "status-panel":
    "Use para loading, error, empty, success e unavailable. Informe título e descrição no idioma do produto; ofereça ação de recuperação para erros e estados vazios. O painel anuncia alterações e não usa apenas cor para comunicar o estado.",
  "data-table":
