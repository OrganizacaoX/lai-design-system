...
test("formulário valida, foca e envia", async ({ page }) => {
...
test("listagem busca, estados e recuperação", async ({ page }) => {
...
test("seletor de período abre e fecha com teclado", async ({
  page,
  isMobile,
}) => {
...
test("diálogo preserva foco e seleção funciona pelo teclado", async ({
  page,
}) => {
...
test("período respeita idioma e limites em atalhos", async ({ page }) => {
  await page.goto("/componentes/date-range-picker");
  const picker = page.getByRole("button", { name: /^Choose reporting dates/ });
  await picker.click();
  await expect(
