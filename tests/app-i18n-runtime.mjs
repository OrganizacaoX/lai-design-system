...
test("instances isolate language/resources and share LAI plural/fallback conventions", async () => {
  ...
  assert.equal(
    a.t("table.selected", { ns: "lai", keySeparator: false, count: 2 }),
    "2 selected",
  );
  assert.equal(
    b.t("table.selected", { ns: "lai", keySeparator: false, count: 1 }),
    "1 seleccionado",
  );
...
test("persistence is opt-in, resilient and detached by dispose", async () => {
  ...
  assert.equal(values.get("app"), "en");
  const explicit = await createAppI18n({
    lng: "pt-BR",
    storageKey: "app",
    storage,
  });
  ...
  explicit.dispose();
  const blocked = await createAppI18n({
    storageKey: "app",
...
test("formatters use explicit currency and timezone", () => {
  ...
  assert.equal(
    createLocaleFormatters("en").date(date, {
      timeZone: "UTC",
      day: "numeric",
    }),
    "15",
