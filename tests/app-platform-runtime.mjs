...
import {
  ...
  createRoute,
  createMemoryHistory,
} from "@organizacaox/lai-design-system/router";
...
test("auth transitions preserve cache on rejection, reset before resuming on success", async () => {
  ...
  const denied = await transitionAuthState({
    ...base,
    action: async () => ({ error: { message: "denied" } }),
  });
  assert(denied.error);
...
test("analytics stays inactive on the server and schemas enforce input", () => {
  ...
  assert.equal(
    createAppAnalytics({ key: "test", host: "http://localhost" }),
    null,
  );
  const schema = z.object({ email: z.email() });
  assert.equal(schema.safeParse({ email: "invalid" }).success, false);
  assert.equal(
    schema.parse({ email: "ana@example.test" }).email,
    "ana@example.test",
  );
});
...
test("AI helper streams text, reasoning and tools through the native client without fetch", async () => {
  ...
  try {
    const client = new ChatClient({
      connection: chat.transport({ delayMs: 0 }),
    });
    ...
    assert(
      parts.some(
        (part) => part.type === "text" && part.content === "All ready.",
      ),
    );
    assert(
      parts.some(
        (part) => part.type === "thinking" && part.content === "Checking.",
      ),
    );
