...
import {
  ...
  useMutation,
  useQueryClient,
} from "@organizacaox/lai-design-system/query";
import { api, contactOptions, type Contact } from "../api";
export const Route = createFileRoute("/contacts/$id")({
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(contactOptions(params.id)),
...
function ContactPage() {
  ...
  const save = useMutation({
    mutationFn: () =>
      api<Contact>(`/contacts/${id}`, {
        method: "PATCH",
        body: { name: name ?? contact.data?.name },
      }),
    onSuccess: async () => {
      await Promise.all([
  ...
  return (
    <>
      <PageHeader title={contact.data?.name ?? "Contato"} />
      <Input
        aria-label="Nome"
        value={name ?? contact.data?.name ?? ""}
