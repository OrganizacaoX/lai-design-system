...
export const Route = createFileRoute("/")({
  ...
  loader: ({ context, deps }) =>
    context.queryClient.ensureQueryData(contactsOptions(deps.q)),
  component: Contacts,
});
function Contacts() {
  ...
  return (
    <>
      ...
      <Input
        ...
        value={q}
        onChange={(event) =>
          void navigate({ search: { q: event.target.value } })
        }
      />
      {contacts.isError ? (
        <StatusPanel state="error" title="Erro ao buscar" />
      ) : (
        <ul>
