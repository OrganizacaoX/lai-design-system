...
import {
  useLaiForm,
  formApiErrors,
} from "@organizacaox/lai-design-system/form";
...
import {
  createAppTour,
  type Driver,
} from "@organizacaox/lai-design-system/tour";
...
import {
  renderWithLai,
  userEvent,
} from "@organizacaox/lai-design-system/testing";
...
function Probe() {
  ...
  return (
    <button onClick={() => setClicked(true)}>
      {language}:{query.data}:{String(clicked)}
    </button>
  );
}
function Platform() {
  ...
  const form = useLaiForm({
    ...
    validators: {
      ...
      onSubmitAsync: async ({ value }) =>
        value.name === "Admin"
          ? formApiErrors({ name: "Nome reservado pela API" })
          : undefined,
    },
  ...
  return (
    <>
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <Check aria-label="Ready" />
        {format(new Date(2026, 0, 2), "MMMM", { locale: ptBR })}
      </motion.p>
      <form
        onSubmit={(event) => {
      ...
      >
        ...
        <form.AppForm>
          <form.SubmitButton>Save platform form</form.SubmitButton>
        </form.AppForm>
      </form>
      ...
      <button
        onClick={async () => {
          ...
          setAuth(
            result.data?.user.name ?? result.error?.message ?? "No session",
          );
        }}
      >
        Load session
      ...
      <button
        onClick={() => {
          const client = createAppAnalytics({
            key: "local-test",
            host: window.location.origin + "/telemetry",
            config: {
              persistence: "memory",
              ...
              before_send: (event) => {
                if (event?.event === "platform_clicked")
                  setAnalytics(event.event);
                return null;
              },
            },
          });
      ...
      <p data-testid="analytics">{analytics}</p>
      <button
        id="tour-target"
        onClick={() => {
          tour?.destroy();
          const next = createAppTour({
            locale: language,
            animate: false,
            steps: [
              {
                element: "#tour-target",
                popover: { title: "LAI tour", description: "Primeiro passo" },
              },
            ],
          });
          setTour(next);
      ...
      <section aria-label="AI conversation">
        <button
          disabled={busy || !chat.next(messages)}
          onClick={() => {
            const next = chat.next(messages);
            if (next) void append(next);
          }}
        >
          Run AI fixture
        </button>
        ...
        {messages.map((message) => (
          <div key={message.id} data-role={message.role}>
            {message.parts.map((part, index) => (
              <p key={index} data-part={part.type}>
      ...
      <button
        onClick={async () => {
          ...
          const view = await renderWithLai(<Probe />, {
            container: target,
            i18n: { lng: "es" },
          });
          try {
