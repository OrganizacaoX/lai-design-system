...
test.beforeEach(async ({ page }) => {
  await page.goto("/tests/fixtures/app-update/index.html");
});

test(
  "card anuncia a versão nova sem interromper o app",
  { tag: ["@component:app-update-card", "@kind:interaction"] },
  async ({ page }) => {
    const card = page
      .getByRole("status")
      .filter({ hasText: "Nova versão disponível" });
    await expect(card).toBeVisible();
    await expect(card).toHaveAttribute("aria-live", "polite");
    await expect(card).toContainText("Atualize quando for conveniente");
    await page
      .getByRole("button", { name: "Atualizar agora", exact: true })
      .click();
    ...
    await page
      .getByRole("button", { name: "Alternar textos", exact: true })
      .click();
...
test(
  "tela mostra o progresso e oferece saída quando a atualização falha",
  { tag: ["@component:app-update-card", "@kind:interaction"] },
  async ({ page }) => {
    await page
      .getByRole("button", { name: "Tela atualizando", exact: true })
      .click();
    const screen = page
      .getByRole("status")
      .filter({ hasText: "Atualizando para a nova versão" });
    ...
    await expect(
      page.getByText("Verifique sua conexão e tente novamente."),
    ).toBeVisible();
    ...
    await page
      .getByRole("button", { name: "Alternar limpar cache", exact: true })
      .click();
    await page
      .getByRole("button", { name: "Limpar cache e reiniciar", exact: true })
      .click();
...
test(
  "card e tela acessíveis nos dois temas",
  { tag: ["@component:app-update-card", "@kind:accessibility"] },
  async ({ page }) => {
    for (const dark of [false, true]) {
      for (const state of ["none", "failed"] as const) {
        if (state === "failed")
          await page
            .getByRole("button", { name: "Tela falha", exact: true })
            .click();
        else
          await page
