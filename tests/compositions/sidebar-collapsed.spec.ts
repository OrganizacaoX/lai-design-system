...
test.describe("sidebar em modo ícone", () => {
  test.skip(
    ...
    "o trilho de ícones só existe no desktop",
  );

  test(
    "colapsada mostra só os ícones, sem sobra de rótulo",
    { tag: ["@component:sidebar", "@kind:layout"] },
    async ({ page }) => {
      ...
      await expect(
        nav.getByRole("link", { name: "Contatos", exact: true }),
      ).toBeVisible();
      ...
      expect(
        await label.evaluate(
          (el, paintedWidth) => new Function("return " + paintedWidth)()(el),
          PAINTED_WIDTH,
        ),
      ).toBeGreaterThan(0);
    },
