...
test(
  "shell navigation, collapse and mobile drawer",
  ...
  async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole("main").locator(":scope > header")).toHaveCount(
      0,
    );
    if (isMobile) {
      ...
      await expect(
        page.getByRole("heading", { name: "Contatos", exact: true }),
      ).toBeVisible();
      await shortcuts
      ...
      await expect(
        dialog.getByRole("button", { name: "Perfil de Ana Silva" }),
      ).toBeVisible();
      await dialog.getByRole("link", { name: "Ajuda", exact: true }).click();
      await expect(dialog).toHaveCount(0);
      await expect(
        page.getByRole("heading", { name: "Ajuda", exact: true }),
      ).toBeVisible();
      await shortcuts
        .getByRole("button", { name: "Menu", exact: true })
        .click();
      ...
      await expect(dialog).toHaveCount(0);
      await expect(
        shortcuts.getByRole("button", { name: "Menu", exact: true }),
      ).toBeFocused();
      await shortcuts
        .getByRole("button", { name: "Menu", exact: true })
        .click();
      await dialog
        .getByRole("button", { name: "Alternar navegação", exact: true })
        .click();
      await expect(dialog).toHaveCount(0);
      await shortcuts
        .getByRole("button", { name: "Menu", exact: true })
      ...
      await page
        .getByRole("dialog")
        .getByRole("link", { name: "Ajuda", exact: true })
        .click();
      await expect(dialog).toHaveCount(0);
    } else {
      ...
      await expect(
        navigation.getByRole("link", { name: "Contatos" }),
      ).toHaveAttribute("aria-current", "page");
      await expect(
        page
          .locator('[data-sidebar="header"]')
          .getByRole("button", { name: "Alternar navegação" }),
      ).toBeVisible();
      await toggle.click();
...
test(
  "shell keyboard skip link and accessibility in both themes",
  { tag: ["@component:app-shell", "@kind:accessibility"] },
  async ({ page }) => {
    ...
    await expect(
      page.locator('[tabindex="-1"]').filter({
        has: page.getByRole("heading", { name: "Visão geral", exact: true }),
      }),
    ).toBeFocused();
...
test(
  "shell controlled state, drawer-only mode and wide content",
  { tag: ["@component:app-shell", "@kind:interaction"] },
  async ({ page, isMobile }) => {
...
test(
  "shell expandable groups select subitems and reveal the collapsed sidebar",
  { tag: ["@component:app-shell", "@kind:interaction"] },
  async ({ page, isMobile }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Novos leads", exact: true }),
    ).toBeVisible();
    if (isMobile) {
      await expect(page.getByRole("dialog")).toHaveCount(0);
      await menu.click();
    ...
    await expect(
      nav.getByRole("link", { name: "Permissões", exact: true }),
    ).toBeVisible();
    await group.click();
    await expect(
      nav.getByRole("link", { name: "Permissões", exact: true }),
    ...
    await expect(
      page.getByRole("heading", { name: "Permissões", exact: true }),
    ).toBeVisible();
