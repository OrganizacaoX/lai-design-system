...
test.beforeEach(async ({ page }) => {
  await page.goto("/tests/fixtures/typography/index.html");
});

test(
  "typography readable at narrow widths with native semantics and refs",
  { tag: ["@component:typography", "@kind:accessibility"] },
  async ({ page }) => {
...
test(
  "shimmer animation, completion, RTL and reduced motion",
  { tag: ["@component:shimmer", "@kind:interaction"] },
  async ({ page }) => {
    const status = page.getByRole("status");
    await expect(status).toHaveCSS("animation-name", "tw-shimmer");
    await expect(status).toHaveCSS("animation-duration", "3.5s");
    await expect(page.getByTestId("rtl")).toHaveCSS(
      "animation-direction",
      "reverse",
    );
