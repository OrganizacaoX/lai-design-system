...
test(
  "selected indicator follows navigation with subtle fill and stronger icon",
  { tag: ["@component:sidebar", "@kind:interaction"] },
  async ({ page, isMobile }) => {
    await page.goto("/examples/app-shell");
    const nav = page.getByRole("navigation", {
      name: isMobile ? "Atalhos de navegação" : "Navegação principal",
      exact: true,
    });
    ...
    const positions = await target.evaluate(async (link) => {
      ...
      for (let i = 0; i < 35; i++) {
        ...
        samples.push({
          ...
          stroke: parseFloat(
            getComputedStyle(link.querySelector("svg")!).strokeWidth,
          ),
        });
      }
      return samples;
    ...
    await expect(target.locator("svg").first()).toHaveCSS(
      "stroke-width",
      "2.5px",
    );
    ...
    expect(
      positions.some((p) => Math.abs(p[axis] - destination![axis]) > 1),
    ).toBe(true);
    expect(Math.abs(positions.at(-1)![axis] - destination![axis])).toBeLessThan(
      1,
    );
    ...
    await expect(
      nav.locator('[data-slot="sidebar-active-indicator"]'),
    ).toHaveCount(1);
  },
);

test(
  "reduced motion places the indicator directly on the selected item",
  { tag: ["@component:sidebar", "@kind:interaction"] },
  async ({ page, isMobile }) => {
