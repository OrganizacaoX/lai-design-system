...
import {
  ...
  resetAppState,
} from "@organizacaox/lai-design-system/app";
import { BetterFetchError } from "@organizacaox/lai-design-system/fetch";
import {
  createStore,
  persist,
  createJSONStorage,
} from "@organizacaox/lai-design-system/store";
...
test("session reset aborts reads and clears query and persisted store state", async () => {
  ...
  const pending = client
    .fetchQuery({
      queryKey: ["old"],
      queryFn: ({ signal }) => api("/slow", { signal }),
    })
    .catch(() => {});
  await ready;
  await resetAppState({
...
test("persisted stores migrate old versions through the LAI middleware exports", () => {
  const data = new Map([
    ["versioned", JSON.stringify({ state: { oldName: "Ana" }, version: 0 })],
  ]);
  const storage = createJSONStorage(() => ({
    getItem: (key) => data.get(key) ?? null,
    setItem: (key, value) => data.set(key, value),
